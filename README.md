## 一个基于react-grid-layout的页面编辑器Demo 
支持在Grid中增加和删除Cell，支持拖拽式的对cell进行布局
每个Cell能够选择嵌入的Widget（react组件）, Widget可定义自己的数据格式，通过react-jsonschema-form生成属性编辑器
编辑完成后，页面布局和Widget信息存入localStorage
